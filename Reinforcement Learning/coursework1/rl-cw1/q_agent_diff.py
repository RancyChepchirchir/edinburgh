...
class QAgent(Agent):
    def __init__(self,
        ...
        # These are only used in callback.
    ...
    def initialise(self, grid):
        ...
        self.grid1 = grid
        self.state = self.define_state(self.grid1, self.grid1)
    ...
    def act(self):
        ...
        def choose_action(state):
            ...
            best_Q = None
            best_action = None
            shuffled_actions = self.getActionsSet()
            ...
            for action in shuffled_actions:
                try:
                    ...
                    return action
                    # self.state_action_reward_dict[(state, action)] = (0., 1)
                    # Q, _ = self.state_action_reward_dict[(state, action)]
                if Q > best_Q:
                    best_Q = Q
                    best_action = action
    ...
    def sense(self, grid):
        ...
        self.grid2 = grid
        self.new_state = self.define_state(self.grid1, self.grid2)
    ...
    def learn(self):
        ...
        if self.use_symmetry:
            ...
            # If flipped state is the same, this just repeats the last operation.
    ...
    def define_state(self, grid1, grid2):
        """ Subset the grid array for cells around the player's car and represent as string.
        """

        def get_sub_grid(grid):
            player_number = 2  # number representing player's car in grid
            player_index = list(grid[0,:]).index(player_number)
            x_min = max(player_index - self.side_visibility, 0)
            x_max = min(player_index + self.side_visibility, grid.shape[1]-1)
            y_max = self.forward_visibility
            return grid1[:(y_max+1), x_min:(x_max+1)]

        return self.array2str(get_sub_grid(grid1) + 3*get_sub_grid(grid2))
