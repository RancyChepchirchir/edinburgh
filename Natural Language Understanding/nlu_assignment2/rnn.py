...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			x_full = make_onehot(x[t], self.vocab_size)
			s[t] = sigmoid(np.dot(self.V, x_full) + np.dot(self.U, s[t-1]))
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			delta_out = make_onehot(d[t], self.vocab_size) - y[t]  # don't include element-wise prod w/ 1's
			delta_in = np.dot(self.W.T, delta_out) * (s[t] * (1 - s[t]))  # broadcasting 1 to vector of 1's
			self.deltaW += np.outer(delta_out, s[t])
			self.deltaV += np.outer(delta_in, make_onehot(x[t], self.vocab_size))
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			for tau in range(1, steps+1):
				if t-tau < 0:
					continue
				# print 't={}, tau{}'.format(t, tau)
				f_net = (s[t-tau] * (1 - s[t-tau]))
				delta_in = np.dot(self.U.T, delta_in) * f_net
				self.deltaV += np.outer(delta_in, make_onehot(x[t-tau], self.vocab_size))
	...
	def compute_loss(self, x, d):
		...
		calc_J = lambda dv, yv: -sum(dv * np.log(yv))  # helper for calculate loss

		d_full = np.array([make_onehot(word_idx, self.vocab_size) for word_idx in d])
		y_full, _ = self.predict(x)
		loss = sum([calc_J(dv, yv) for dv, yv in zip(d_full, y_full)])
	...
	def generate_sequence(self, start, end, maxLength, removeIndices=None):
		'''
		...
		while sequence[-1] != end and len(sequence) < maxLength:
			y = self.predict(sequence)[0][-1]
			if removeIndices is not None:
				y[removeIndices] = 0
			new_word_idx = multinomial_sample(y)
			sequence.append(new_word_idx)
			# print 'sequence:\n{}\n'.format(sequence)
			loss += -np.log(y[new_word_idx])
...
if __name__ == "__main__":
	...
	if mode == "estimate":
		...
		for hdim in [25, 50]:
		    for back_steps in [0, 2, 5]:
		        for learning_rate in [0.5, 0.1, 0.05]:
					r = RNN(vocab_size, hdim)
					loss = r.train(X, D, X_dev, D_dev, learning_rate=learning_rate, back_steps=back_steps)
					if loss < best_loss:
						best_loss = loss
						best_hdim = hdim
						best_lookback = back_steps
						best_lr = learning_rate
						best_params = (hdim, back_steps, learning_rate)
	...
	if mode == "train":
		...
		# Save weights
		rnn_folder = 'rnn'
		np.save(rnn_folder + '/rnn.U.npy', r.U)
		np.save(rnn_folder + '/rnn.V.npy', r.V)
		np.save(rnn_folder + '/rnn.W.npy', r.W)
		
		# Compute losses
		run_loss = r.compute_mean_loss(*seqs_to_lmXY(S_dev))
		adjusted_loss = adjust_loss(run_loss, fraction_lost, q)

		# To run: python rnn.py train data 25 0 0.5
	...
	if mode == "generate":
		...
		removeIndices = [word_to_num['UUUNKKK']]
		sequence, loss = r.generate_sequence(start, end, maxLength, removeIndices)
		# print 'sequence: \n{}\n'.format(' '.join([num_to_word[idx] for idx in sequence if num_to_word[idx] != 'UUUNKKK']))
		print 'sequence: \n{}\n'.format(' '.join([num_to_word[idx] for idx in sequence]))
		print 'loss: {}'.format(loss)

		# To run: python rnn.py generate data rnn 25
